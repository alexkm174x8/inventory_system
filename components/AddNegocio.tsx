...
const AddNegocio: React.FC<AddNegocioProps> = ({ onClose, onNegocioAdded }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (adminError) {
        throw new Error('Error al crear el negocio. Por favor, contacte al administrador del sistema.');
      }
      ...
      if (updateError) {
        toast({
          variant: "destructive",
          title: "Advertencia",
          description: "El negocio se creó pero hubo un error al actualizar los metadatos del usuario.",
        });
      }

      toast({
        title: "Éxito",
        description: "Negocio agregado exitosamente",
      });

      onNegocioAdded();
      onClose();
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : 'Error desconocido al agregar negocio';
      toast({
        variant: "destructive",
        title: "Error",
